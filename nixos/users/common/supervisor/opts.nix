...
rec {
  supervisorctl = {
    server = {
      type = "unix"; # or http
      url =
        if supervisorctl.server.type == "unix" then
          unix_http_server.file
        else
          "${inet_http_server.ip_addr}:${inet_http_server.port}";
    };
    auth = if supervisorctl.server.type == "unix" then unix_http_server.auth else inet_http_server.auth;
  };
  unix_http_server = {
  ...
  generated = {
    ...
    inet_http_server_config =
      if !inet_http_server.enable then
        ""
      else
        ''
          [inet_http_server]
