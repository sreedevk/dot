return function()
  ...
  local function setup_custom_lsp(conf)
    vim.api.nvim_create_autocmd('FileType', {
      desc = "Configurations on LSP Attach",
      pattern = conf.filetypes,
      callback = function()
        vim.lsp.start(generate_config(conf))
      end
    })
  end
  ...
  local function setup_lsp(conf)
    if conf.custom then
      setup_custom_lsp(conf)
    else
      lspconf[conf.name].setup(generate_config(conf))
    end
  end
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      vim.keymap.set("n", "]d", vim.diagnostic.goto_next, keymap_opts(event, "Next Diagnostic Message"))
      vim.keymap.set("n", "[d", vim.diagnostic.goto_prev, keymap_opts(event, "Previous Diagnostic Message"))
